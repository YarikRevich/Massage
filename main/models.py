...
class ModificatedUserManager(models.Manager):

	def get_static_info(self, request: object, option):
		"""Returns static info about user."""

		try:
			mod_user_obj = super().get_queryset().get(number_of_user=request.COOKIES["*1%"])
		except KeyError:
			try:
...
class ModificatedUser(models.Model):
	...
	"""
	objects = ModificatedUserManager()
	...
	number_of_made_records = models.IntegerField(
		verbose_name="Количество сделаних записей", default=0
	)
	number_of_visites = models.IntegerField(
		verbose_name="Количество визитов пользователя", default=0
	)
